...
- Design is iterated by comment: keep visual decisions in `css/tokens.css` so each comment is a small diff. Draft 1 (2026-09-13) reuses the Chess Master language (metar-taf: white ground, Helvetica Neue, small uppercase letter-spaced labels, hairline borders, square corners). Draft 2 (same day, owner's comment): the accent is a **dark red** (`#4d0f14`, Chess Master's green at the same saturation and lightness, hue 355°) so the two apps are siblings in different colours; board good/bad marks stay green/red. The board is pale kaya wood with thin ink lines, slate and shell stones (`--gb-*`, `--stone-*` tokens). Lesson portraits get the same old-film treatment as Chess Master (`.board-aside`), **only on lessons built around a famous game or player**, never as decoration; photos only public-domain or CC from Wikimedia Commons, credited in `sources:`.
