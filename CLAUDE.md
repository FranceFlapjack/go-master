...
- **The Master series switcher** (`js/family.js`, 2026-09-15): hovering or clicking the logo opens a rounded glass bubble (Apple-style: blur, saturation, a rim light; `--bubble-*` tokens) listing the *other* apps of the series — logo in its own accent and name only, never the app you are in — and ending with a dashed silhouette, "Coming soon", for the next Master. It comes out of the logo's lower corner (the bubble is positioned against the logo, not the brand block). `FAMILY` in that file is the whole registry and **must be identical in every app of the series**: adding a Master is one entry, copied to the other apps. Links are `../<app>/` on GitHub Pages (the apps are siblings) and the `dev` port on localhost. Nothing is fetched from a sibling and no progress is shared.
...
node scripts/review-test.mjs      # the review ladder and the lesson-fence reader (js/review.js, js/tries.js)
```
...
- `js/rules/go.js` the rules: board, chains and liberties, capture, suicide, **ko: simple ko as the fast path and positional superko behind it** (Zobrist hashes of every position reached; `check()` returns reason `superko`; the bot's `FastBoard` keeps simple ko only, and `scripts/bot-test.mjs` counts the difference), passes, **area (Chinese) scoring with komi 7.5** and a dead-stone list; `handicapPoints` (fixed placement, the traditional order). Coordinates: `A1`…`T19` (no I) in lessons, `aa`…`ss` in SGF.
...
- `js/sgf.js` SGF parser (keeps the whole tree) + `loadSgf` (main line with positions) + `parseSolution` (the puzzle tree syntax). `js/sgf-viewer.js` annotated game viewer with keyboard nav over one line of the tree; where a node branches, the move list shows the other variations as "or …" buttons and a Variations row appears under the comment (keys 1–9 pick); `lineOf`/`replayLine` in `js/sgf.js` walk a line by a choice map.
- `js/review.js` the **daily review**: a Leitner ladder (boxes of 1, 3, 7, 16, 35 days) over the problems already solved — right moves a problem up a box, wrong drops it to box 1, and a problem joins the ladder when it is first solved (first try starts a box higher). The table lives inside the progress object, so export/import carries it; `review.ensureSeeded()` (called at boot) puts everything solved before the feature existed on the ladder, spread over two weeks, and never delays what fits in one session. `js/review-page.js` is the `#/review` page (kept separate so `js/exercise.js` can join a solve to the ladder without an import cycle); it never copies a problem — an id is `<track>/<slug>#<n>`, so it fetches the lesson and takes its nth `try` fence (`js/tries.js`), and an entry whose fence has gone is dropped. `mountExercise(..., { review: true, onMiss })` replays a solved problem without re-recording it. The intervals are the standard Leitner spread, not tuned or claimed optimal.
