...
5. **Joseki** (5) — **done 2026-09-14**: 4-4 knight's approach (three quiet answers, slide–block–extend), attach-and-extend, 3-4 basics (enclosures, Shusaku's diagonal), the classical 3-3 invasion (both blocks), when to leave (Wikipedia's proverbs, two departures from the model games). Sequences are move lists from Kogo's Joseki Dictionary (its terms: ask before distributing, so the file is not vendored and its comments are not quoted), checked for legality; the exercises play each joseki through. 12 problems. Not covered: pincers, the post-2016 3-3 lines (not in the 2014 dictionary).
6. **Direction of play** (7) — **done 2026-09-16**: urgent before big, attack to gain (not to kill), influence and territory, thickness, invading and reducing, good and bad shape, ko fights and threats. 24 problems. The first track with no oracle for its main ideas: each lesson's tactical kernel is machine-checked (capture search with `refute:` on the tempting big point, life search on the group that needs a move and on invasions of closed areas, the scorer on every counted diagram including the value of a ko and of a threat), and the strategic judgement follows a quoted source — Wikipedia's *Go strategy and tactics*, *Go proverb*, *Shape (Go)*, *Ko fight* (fetched, not recalled) — or the move played in a verified record (AlphaGo–Lee Sedol g2 move 37). Each lesson's `sources:` says which is which. `scripts/capture-explore.mjs` added for authoring. Not covered: pincers, sabaki, the theory of *The Direction of Play* (Kajiwara) beyond its name.
   **The endgame** (4) — **done 2026-09-16**: sente and gote, counting the value of a move, endgame tesuji (monkey jump, the hane that dies, the first-line placement), neutral points and the final count. 12 problems. This is the scorer's track: every value is the difference between two counted diagrams, each checked by `score:` (the checker caught two miscounted captions on the way). The throw-in promised by the races lesson is explained but not set as a problem — the capture search cannot read it — and the lesson says so.
7. **Study a whole game** (3) — **done 2026-09-18**: the ear-reddening game (Gennan Inseki – Shusaku, 1846), Go Seigen – Honinbo Shusai 1933 (record fetched from Brouwer's archive and checked against Wikipedia's Go Seigen article: colours, start date, W+2, the 3-3/4-4/tengen opening, move 160; comments ours), Lee Sedol – AlphaGo 2016 game 4. 5 problems, each accepting the move played in the game. No engine judged any move and each lesson says so; the 1933 record is the fifth game file.
